function setup_sfun_zeromq_image_m_mask(block)
% This script creates a mask for the 'sfun_zeromq_image_m' S-Function block.
% If no block path is provided, it acts on the currently selected block (gcb).
%
% To use it manually:
% 1. Open your Simulink model.
% 2. Select the 'sfun_zeromq_image_m' block.
...
if ~strcmp(sfun_name, 'sfun_zeromq_image_m')
    error('This script is intended for "sfun_zeromq_image_m". The selected block is "%s".', sfun_name);
end
...
param = mask.addParameter('Type', 'checkbox', 'Name', 'save_debug_image', 'Prompt', 'Save last received image (received_last.jpg)');
param.Container = 'DebuggingTab';
param.Value = 'off';
...
mask.Initialization = sprintf('%s;%s;%s;%s;%s;%s;%s', ...
  ...
  'if strcmp(get_param(gcb,''save_debug_image''),''on''), save_debug_image_str=''true''; else, save_debug_image_str=''false''; end', ...
  'qs = char(39);', ...
  ['fmt = [qs ''%s'' qs '', '' qs ''%s'' qs '', %s, %s, %s, %s, %s, %s, %s, %s''];'], ...
  ['params_str = sprintf(fmt, get_param(gcb,''address''), get_param(gcb,''camera_id''), ', ...
     'get_param(gcb,''imgHeight''), get_param(gcb,''imgWidth''), channel_num_str, ', ...
     'get_param(gcb,''timeout_ms''), get_param(gcb,''sample_time''), bind_mode_str, ', ...
     'enable_logging_str, save_debug_image_str);'], ...
...
disp('Mask for sfun_zeromq_image_m has been created/updated successfully.');
