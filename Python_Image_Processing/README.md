...
-----------------
前提:
- Windows / macOS / Linux 上で Python 3.8+ を使用することを想定
- UE5 側で ZeroMQ を用い、トピック（例: `Camera01`）付きの multipart メッセージ [topic, data] を送信すること

推奨インストール方法（仮想環境推奨）:
```bash
# 仮想環境作成（任意）
python -m venv .venv
# 仮想環境有効化 (Windows PowerShell の例)
.venv\Scripts\Activate.ps1

# 必要パッケージのインストール
pip install numpy opencv-python pyzmq
# または、プロジェクト全体の requirements がある場合:
# pip install -r ../requirements.txt
```

使い方（簡単な実行手順）
-----------------------
1. UE5（または別の送信側）が下記のフォーマットで画像を送信していることを確認します:
   - ZeroMQ PUB → Python の SUB に接続
   - メッセージは multipart: [topic (文字列), data (バイト列)]
   - data はチャネル優先のバイト配列 (C, H, W) のフラット配列（チャネル＝3、uint8）
     - 本リポジトリの `main.py` はデフォルトで 1024x1024, 3ch を想定しています
2. 本モジュールを起動:
```bash
python Python_Image_Processing/main.py
```
3. ウィンドウが開き、受信画像・特徴表示が行われます。`q` キーで終了、Ctrl+C でも停止可能。
